...
class cnChannelSelectionExtraInfo(Converter, object):
	Reference = 1
	...
	def getText(self):
		...
		if self.type == self.Reference:
		...
		elif (self.type >= self.NextEventTitle and self.type <= self.NextEventFull):
			eventNext = ''
			...
			if (len(list) > 0):
				...
				print eventNext
				#eventNext[4] - title
				#eventNext[1] - start time
				#eventNext[2] - duration
				if eventNext[4]:
			...
			if (info):
					...
					if transponderData.has_key('orbital_position'):
						try:
							orb = {3590: ' (1.0W)',
								...
								48: ' (4.8E)',
								30: ' (3.0E)'
							}[transponderData['orbital_position']]
						except:
							orb = ''
				except:
					pass
				...
				if self.type == self.TInfoFull:
					return "%s %s %s %s %s" % (fq , pol, fec, sr, orb)
		return ''
